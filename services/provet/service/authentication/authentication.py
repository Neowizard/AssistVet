...
class Authentication:
    ...
    async def login(self) -> Optional[list[dict]]:
        ...
        await self._browser.enter_input("id_username", self._config.provet_username)
        await self._browser.enter_input("id_password", self._config.provet_password)
        login_time = datetime.datetime.now()
        await self._browser.click_button("id_btn_login", expect_redirect=True)
        url = self._browser.get_page_url()
        if endpoints.check_url_endpoint(url, endpoints.MFA_CHALLENGE_ENDPOINT):
            await self._mfa.resolve_mfa(self._browser, login_time)

        url = self._browser.get_page_url()
        if endpoints.check_url_endpoint(url, endpoints.DASHBOARD_ENDPOINT):
            return await self._browser.get_page_cookies(Authentication.SESSION_COOKIE_NAMES)
        logger.error(f"Failed to login to {url}. "
                     f"Expected landing endpoint: {endpoints.DASHBOARD_ENDPOINT}. "
                     f"Actual: {url}")
        return None

    async def get_session(self) -> list[Cookie]:
        ...
        if len(session_cookies) < len(self.SESSION_COOKIE_NAMES):
            ...
            CookieStore.store(self._config.provet_url, login_cookies)
            session_cookies = CookieStore.retrieve(self._config.provet_url, self.SESSION_COOKIE_NAMES)
